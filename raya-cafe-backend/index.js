...
// API AMBIL MENU & PENGECEKAN STOK BOM
...
// API CHECKOUT & TRANSAKSI DATABASE
...
// SINKRONISASI KE DATABASE KASIR LAMA
app.post("/api/admin/orders/:id/print", async (req, res) => {
  ...
  try {
    ...
    res.json({
      message: "Tagihan dicetak & sukses dikirim ke Database Kasir Lama!",
    });
  } catch (error) {
    res
      .status(400)
      .json({ error: "Gagal terhubung ke Database Legacy: " + error.message });
  }
});
...
app.post("/api/admin/reservations/manual", async (req, res) => {
  try {
    ...
    if (!name || !phone || !date || !pax) {
      return res
        .status(400)
        .json({ error: "Nama, No HP, Tanggal, dan Pax wajib diisi!" });
    }
    ...
    if (type === "event") {
      ...
      if (
        Math.ceil(
          (resDay.getTime() - today.getTime()) / (1000 * 60 * 60 * 24),
        ) < 3
      ) {
        return res
          .status(400)
          .json({ error: "Reservasi Event wajib minimal H-3 dari sekarang!" });
      }
    }
    ...
    res.json({
      message: "Reservasi manual berhasil ditambahkan!",
      data: result,
    });
  } catch (error) {
    res.status(400).json({ error: error.message });
...
app.post("/api/admin/reservations/:id/confirm-event", async (req, res) => {
  try {
    ...
    await prisma.$transaction(async (tx) => {
      const targetResv = await tx.reservations.findUnique({
        where: { id: resvId },
      });

      const duaJamSebelum = new Date(
        targetResv.reservation_date.getTime() - 2 * 60 * 60 * 1000,
      );
      const duaJamSesudah = new Date(
        targetResv.reservation_date.getTime() + 2 * 60 * 60 * 1000,
      );
      ...
      if (bentrok)
        throw new Error(
          `Meja ${table_id} sudah dipakai oleh ${bentrok.customer_name} pada jam ${bentrok.reservation_date.toLocaleTimeString("id-ID")}.`,
        );
      ...
      if (menu_items && menu_items.length > 0) {
        ...
        for (const item of menu_items) {
          ...
          for (const resep of menu.recipe_bom) {
            const kebutuhan =
              Number(resep.quantity_required) * targetResv.pax_count * item.qty;
            if (Number(resep.raw_materials.current_stock) < kebutuhan)
              throw new Error(
                `Stok ${resep.raw_materials.name} tidak cukup untuk memasak ${menu.name}!`,
              );
            await tx.raw_materials.update({
              where: { id: resep.raw_material_id },
              data: { current_stock: { decrement: kebutuhan } },
...
app.post("/api/admin/reservations/:id/confirm-meja", async (req, res) => {
  try {
    ...
    await prisma.$transaction(async (tx) => {
      ...
      if (bentrok)
        throw new Error(
          `Meja ${tableId} sudah dipakai oleh ${bentrok.customer_name} pada jam ${bentrok.reservation_date.toLocaleTimeString("id-ID")}.`,
        );
      await tx.reservations.update({
        where: { id: resvId },
...
app.post("/api/admin/reservations/:id/print", async (req, res) => {
  try {
    ...
    await prisma.$transaction(async (tx) => {
      const resv = await tx.reservations.findUnique({
        where: { id: parseInt(req.params.id) },
      });
      ...
      if (resv.table_id) {
        ...
        const tableRecord = await tx.tables.findFirst({
          where: { table_number: resv.table_id },
        });
        if (tableRecord) {
          validTableId = tableRecord.id;
        } else {
    ...
    res.json({
      message:
        "Tagihan Event berhasil dicetak, masuk ke Laporan, dan terkirim ke Database Kasir Lama!",
    });
  } catch (error) {
    res
      .status(400)
  ...
  try {
    ...
    orders.forEach((o) => {
      ...
      o.order_items.forEach((i) => {
        if (i.menu_items) {
          itemCounts[i.menu_items.name] =
            (itemCounts[i.menu_items.name] || 0) + i.quantity;
        }
      });
    });
...
app.get("/api/admin/inventory", async (req, res) => {
  try {
    res.json(await prisma.raw_materials.findMany());
  } catch (error) {
    res.status(400).json({ error: error.message });
  }
});
...
app.get("/api/admin/inventory-history", async (req, res) => {
  try {
    res.json(
...
app.post("/api/admin/promotions", async (req, res) => {
  ...
  try {
    if (!menu_item_ids || !Array.isArray(menu_item_ids))
      throw new Error("menu_item_ids harus array!");
    ...
    res.json({
      message: `Diskon berhasil ditambahkan ke ${result.count} menu!`,
    });
  } catch (error) {
    res.status(400).json({ error: error.message });
  }
...
app.get("/api/admin/feedback", async (req, res) => {
  try {
    const feedbacks = await prisma.feedback.findMany({
      include: { menu_items: true, orders: { include: { tables: false } } },
      orderBy: { created_at: "desc" },
    });
    res.json(feedbacks);
  } catch (error) {
...
app.post("/api/admin/users", async (req, res) => {
  ...
  try {
    ...
    await prisma.users.create({
      data: { username, password_hash: hashPassword, role },
    });
    res.json({ message: "Akun pegawai berhasil dibuat!" });
  } catch (error) {
    res
      .status(400)
      .json({ error: "Username sudah terpakai atau input tidak valid." });
  }
});
