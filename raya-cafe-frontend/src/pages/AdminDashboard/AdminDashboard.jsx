...
function AdminDashboard() {
  ...
  const [manualResvForm, setManualResvForm] = useState({
    name: "",
    phone: "",
    date: "",
    pax: "",
  ...
  const [showPrintEventModal, setShowPrintEventModal] = useState(false);
  const [eventTotalAmount, setEventTotalAmount] = useState("");
  ...
  const fetchData = () => {
    ...
    axios
      ...
      .then((res) => {
        ...
        if (
          newActiveOrders.length > prevOrderCount.current &&
    ...
    axios
      .get(`${API_URL}/api/admin/reservations`)
      .then((res) => {
        const newResv = res.data.filter((r) => r.status === "pending_wa");
        if (
          newResv.length > prevResvCount.current &&
          prevResvCount.current !== 0
        ) {
          playNotificationSound();
        }
    ...
    if (userRole === "admin" || userRole === "owner") {
      axios
        ...
        .then((res) => {
          ...
          const currentLowStock = newData.filter(
            (item) => item.current_stock <= item.min_stock_level,
          ).length;
          if (
            currentLowStock > prevLowStockCount.current &&
    ...
    if (activeTab === "promotions")
      axios
        .get(`${API_URL}/api/admin/promotions`)
        .then((res) => setPromotions(res.data))
        .catch((err) => console.log(err));
    if (activeTab === "feedback")
      axios
        .get(`${API_URL}/api/admin/feedback`)
  ...
  }, [isAuthenticated, activeTab, showInvHistory, reportDate]);
  const updateOrderStatus = (id, newStatus) => {
    axios
      .patch(`${API_URL}/api/admin/orders/${id}/status`, { status: newStatus })
      .then(() => fetchData());
  };
  const triggerLegacyPrint = (order) => {
  ...
  const submitEventConfirmation = () => {
    ...
    if (eventMode === "paket" && !selectedPackageId)
      return alert("Pilih paket event dari daftar!");
    if (eventMode === "pilih" && eventCart.length === 0)
      return alert(
        "Pilih minimal satu menu untuk dimasukkan ke keranjang event!",
      );
    const payload = {
    ...
    axios
      .post(
        `${API_URL}/api/admin/reservations/${selectedResv.id}/confirm-event`,
        payload,
      )
      .then((res) => {
  ...
  const confirmMejaBiasa = () => {
    ...
    axios
      .post(
        `${API_URL}/api/admin/reservations/${selectedResv.id}/confirm-meja`,
        { table_id: selectedTableId },
      )
      .then((res) => {
        alert(res.data.message);
  ...
  // MENYELESAIKAN MEJA BIASA SECARA MANUAL
  ...
  const submitManualResv = () => {
    ...
    if (manualResvForm.type === "event") {
      ...
      if (resDate < minDate)
        return alert(
          "❌ Maaf, Reservasi Event wajib dilakukan minimal H-3 dari sekarang!",
        );
    }
    axios
      .post(`${API_URL}/api/admin/reservations/manual`, manualResvForm)
      .then((res) => {
        alert("Sukses!");
        setShowManualResv(false);
        fetchData();
      })
      .catch((err) =>
        alert(err.response?.data?.error || "Gagal menyimpan reservasi manual"),
      );
  };
  ...
  const confirmEventPrint = () => {
    ...
    if (selectedResv?.event_choice?.startsWith("paket_")) {
      ...
      const items = itemsStr
        .split(",")
        .map((pair) => {
          const [mId, mQty] = pair.split(":");
          const menu = menus.find((m) => m.id === parseInt(mId));
          return menu ? `${mQty}x ${menu.name}` : null;
        })
        .filter(Boolean);
  ...
  const submitStockUpdate = () => {
    ...
    axios
      .patch(`${API_URL}/api/admin/inventory/${selectedMaterial.id}`, {
        stock: newStockVal,
        reason: stockReason,
        updated_by: username,
        role: userRole,
  ...
  const submitPromo = () => {
    if (
      promoForm.menu_item_ids.length === 0 ||
      !promoForm.value ||
      !promoForm.end_date
    )
      return alert("Pilih minimal 1 menu dan isi semua data!");
    axios.post(`${API_URL}/api/admin/promotions`, promoForm).then(() => {
      alert("Sukses!");
  ...
  const submitUser = () => {
    ...
    axios
      .post(`${API_URL}/api/admin/users`, userForm)
      .then((res) => {
        alert(res.data.message);
      ...
      .catch((err) => alert(err.response?.data?.error || "Gagal membuat akun"));
  };
  // --- POS KASIR MANUAL ---
  ...
  const addPosItem = (menu) => {
    ...
    if (exist) {
      setPosCart(
        posCart.map((i) => (i.id === menu.id ? { ...i, qty: i.qty + 1 } : i)),
      );
    } else {
  ...
  const reducePosItem = (menu) => {
    ...
    if (exist) {
      if (exist.qty === 1) {
        setPosCart(posCart.filter((i) => i.id !== menu.id));
      } else {
        setPosCart(
          posCart.map((i) => (i.id === menu.id ? { ...i, qty: i.qty - 1 } : i)),
        );
        ...
        <div className="login-box">
          ...
          <p
            style={{
              marginTop: "20px",
              fontSize: "0.78rem",
              color: "rgba(200,170,145,0.45)",
            }}
          >
            © 2026 Raya Cafe Madiun
          </p>
        </div>
      </div>
    );
  return (
    <div className="admin-container">
      ...
      {showResvModal && (
        <div className="modal-overlay">
          <div className="modal-content">
            <h3>
              Konfirmasi{" "}
              {selectedResv.reservation_type === "event" ? "EVENT" : "MEJA"}:{" "}
              {selectedResv.customer_name}
            </h3>
            <p>
              Untuk <strong>{selectedResv.pax_count} orang</strong>.
            </p>
                ...
                <>
                  ...
                  {eventMode === "pilih" && (
                    ...
                    >
                      ...
                      >
                        <select
                          value={eventMenuDropdown}
                          onChange={(e) => setEventMenuDropdown(e.target.value)}
                        >
                        ...
                        <button
                          ...
                          onClick={() => {
                            ...
                            const m = menus.find(
                              (x) => x.id === parseInt(eventMenuDropdown),
                            );
                            const exist = eventCart.find((x) => x.id === m.id);
                            if (exist)
                              setEventCart(
                                eventCart.map((x) =>
                                  x.id === m.id ? { ...x, qty: x.qty + 1 } : x,
                                ),
                              );
        ...
        <div className="modal-overlay">
          <div className="modal-content">
            ...
            <p style={{ color: "#666", marginBottom: "5px" }}>
              Untuk: <strong>{selectedResv?.customer_name}</strong>{" "}
              &nbsp;|&nbsp; <strong>{selectedResv?.pax_count} Pax</strong>
            </p>
            <p
              style={{
                color: "#888",
            ...
            >
              Sistem telah menghitung otomatis. Ubah jika ada diskon khusus.
            </p>
            <div className="modal-form">
              ...
              <input
                ...
                value={eventTotalAmount}
                onChange={(e) => setEventTotalAmount(e.target.value)}
              />
              <div className="modal-actions">
                <button
                  className="btn-secondary"
                  onClick={() => setShowPrintEventModal(false)}
                >
                  Batal
                </button>
                <button className="btn-ready" onClick={submitEventPrint}>
                  Preview Nota &rarr;
                </button>
              </div>
            </div>
        ...
        <div className="modal-overlay" style={{ zIndex: 2000 }}>
          ...
          >
            ...
            >
              ...
              >
                <span>
                  ...
                  <strong style={{ color: "#333" }}>
                    {previewOrder.tables?.table_number || previewOrder.table_id}
                  </strong>
                </span>
                <span>
              ...
              <div
                style={{
                  borderTop: "2px dashed #d4a96a",
                  borderBottom: "2px dashed #d4a96a",
                  padding: "12px 0",
                  margin: "12px 0",
                }}
              >
              ...
              {Number(previewOrder.discount_amount) > 0 && (
                ...
                >
                  <span>Subtotal</span>
                  <span>
                    Rp {Number(previewOrder.subtotal).toLocaleString("id-ID")}
                  </span>
                </div>
              )}
              {Number(previewOrder.discount_amount) > 0 && (
                ...
                >
                  ...
                  <span>
                    - Rp{" "}
                    {Number(previewOrder.discount_amount).toLocaleString(
                      "id-ID",
                    )}
                  </span>
                </div>
              ...
              >
                <span>TOTAL</span>
                <span>
                  Rp {Number(previewOrder.final_total).toLocaleString("id-ID")}
                </span>
              </div>
              <p
                style={{
            ...
            >
              ...
              >
                ...
                >
                  ...
                  <span style={{ fontWeight: "bold" }}>
                    {new Date(selectedResv.reservation_date).toLocaleDateString(
                      "id-ID",
                ...
                {(() => {
                  if (selectedResv.event_choice?.startsWith("paket_")) {
                    const pId = parseInt(
                      selectedResv.event_choice.split("_")[1],
                    );
                    const pkg = packages.find((p) => p.id === pId);
                    if (!pkg) return null;
                    return (
                      ...
                      >
                        ...
                        >
                          {pkg.description ? (
                            pkg.description.split(",").map((item, i) => (
                              ...
                              >
                                <span
                                  style={{ color: "#a0522d", flexShrink: 0 }}
                                >
                                  •
                                </span>
                                <span>{item.trim()}</span>
                        ...
                        <div
                          style={{
                            ...
                            color: "#888",
                            marginTop: "6px",
                            borderTop: "1px dashed #e8c99a",
                            paddingTop: "6px",
                          }}
                        >
                          Rp {Number(pkg.price).toLocaleString("id-ID")} /pax ×{" "}
                          {selectedResv.pax_count} pax
                        </div>
                      </div>
                    );
                  } else if (selectedResv.event_choice?.startsWith("pilih_")) {
                    const itemsStr = selectedResv.event_choice.replace(
                      "pilih_",
                      "",
                    );
                    const items = itemsStr
                      ...
                      .map((pair) => {
                        ...
                        const menu = menus.find((m) => m.id === parseInt(mId));
                        return menu
                          ? { name: menu.name, qty: parseInt(mQty) }
                          : null;
                      })
                      .filter(Boolean);
                    return (
              ...
              >
                <span>TOTAL</span>
                <span>
                  Rp {Number(eventTotalAmount).toLocaleString("id-ID")}
                </span>
              </div>
              <p
                style={{
              ...
              {selectedResv.event_choice?.startsWith("paket_") && (
                ...
                >
                  ⚠️ <strong>Pengingat Stok:</strong> Kurangi stok bahan baku
                  secara manual di tab <strong>📦 Stok</strong> sesuai jumlah
                  pax event ini.
                </div>
              )}
            </div>
          ...
          <div className="modal-content">
            ...
            <div className="modal-form">
              <input
                ...
                onChange={(e) =>
                  setManualResvForm({ ...manualResvForm, name: e.target.value })
                }
              />
              <input
                type="text"
              ...
              <input
                type="datetime-local"
                onChange={(e) =>
                  setManualResvForm({ ...manualResvForm, date: e.target.value })
                }
              />
              <input
                type="number"
                placeholder="Jumlah Orang (Pax)"
                onChange={(e) =>
                  setManualResvForm({ ...manualResvForm, pax: e.target.value })
                }
              />
              <select
                onChange={(e) =>
                  setManualResvForm({
                    ...manualResvForm,
                    type: e.target.value,
                    event_choice: e.target.value === "event" ? "paket" : null,
                  })
                }
              >
              ...
              <div className="modal-actions">
                <button
                  className="btn-secondary"
                  onClick={() => setShowManualResv(false)}
                >
                  Batal
                </button>
                <button className="btn-process" onClick={submitManualResv}>
                  Simpan
                </button>
              </div>
            </div>
      ...
      {showStockModal && (
        <div className="modal-overlay">
          <div className="modal-content">
            <h3>Update Stok: {selectedMaterial?.name}</h3>
            <div className="modal-form">
              <input
                type="number"
                value={newStockVal}
              ...
              <input
                type="text"
                placeholder="Alasan (Contoh: Tumpah, Basi)..."
                value={stockReason}
          ...
          <div className="modal-content">
            ...
            <div className="modal-form">
              ...
              >
                ...
                >
                  Pilih Menu (Bisa Lebih Dari Satu)
                </div>
                {menus.map((m) => (
                  <label
                    key={m.id}
                  ...
                  >
                    <input
                      type="checkbox"
                      value={m.id}
                      checked={promoForm.menu_item_ids.includes(
                        m.id.toString(),
                      )}
                      onChange={(e) => {
                        ...
                        setPromoForm((prev) => ({
                          ...prev,
                          menu_item_ids: checked
                            ? [...prev.menu_item_ids, val]
                            : prev.menu_item_ids.filter((id) => id !== val),
                        }));
                      }}
              ...
              <div className="modal-actions">
                ...
                <button className="btn-ready" onClick={submitPromo}>
                  Simpan
                </button>
              </div>
            </div>
          ...
          <div className="modal-content">
            ...
            <div className="modal-form">
              ...
              <div className="modal-actions">
                ...
                <button className="btn-ready" onClick={submitUser}>
                  Simpan Akun
                </button>
              </div>
            </div>
      ...
      <header className="admin-header">
        ...
        <div className="header-actions">
          <Link to="/" className="link-btn" target="_blank">
            Lihat PWA
          </Link>
      ...
      <div className="admin-tabs">
        <button
          className={activeTab === "orders" ? "adm-tab active" : "adm-tab"}
        ...
        {(userRole === "admin" || userRole === "owner") && (
          <>
            <button
              className={
                activeTab === "inventory" ? "adm-tab active" : "adm-tab"
              }
              onClick={() => setActiveTab("inventory")}
              style={{ verticalAlign: "top", padding: "6px 20px" }}
            >
              <div
                style={{
                  display: "flex",
        ...
        {userRole === "owner" && (
          <>
            ...
            <button
              className={activeTab === "reports" ? "adm-tab active" : "adm-tab"}
          ...
          activeTab !== "inventory" && (
            <div
              style={{
                ...
                right: "30px",
                background:
                  "linear-gradient(135deg, var(--accent-danger), #a93226)",
                color: "white",
                padding: "16px 24px",
            ...
            >
              ...
              <div>
                ...
                <p style={{ margin: 0, fontSize: "0.85rem", opacity: 0.9 }}>
                  Ada <strong>{lowStockCount} bahan baku</strong> yang stoknya
                  menipis/habis.
                </p>
              </div>
              <button
            ...
            {orders.map((order) => (
              <div key={order.id} className="order-card-admin">
                <div className="order-card-header">
                  <h3>Meja {order.tables?.table_number || order.table_id}</h3>
                ...
                <div className="order-card-body">
                  ...
                  <h4 className="order-total">
                    Total: Rp{" "}
                    {Number(order.final_total).toLocaleString("id-ID")}
                  </h4>
                </div>
                <div className="order-card-actions">
                  ...
                  {order.status === "served" && (
                    <button
                      ...
                      style={{
                        background:
                          "linear-gradient(135deg,var(--coffee-latte),var(--coffee-caramel))",
                        color: "var(--coffee-espresso)",
                      }}
                      onClick={() => triggerLegacyPrint(order)}
                    >
                      🖨️ Cetak Tagihan
                    </button>
                  )}
            ...
            {orders.length === 0 && (
              <div
                style={{
                  ...
                  border: "1.5px dashed rgba(92,61,46,0.12)",
                }}
              >
                ...
                <p style={{ color: "var(--text-muted)", fontSize: "0.95rem" }}>
                  Tidak ada pesanan aktif saat ini. Halaman akan memperbarui
                  secara otomatis.
                </p>
              </div>
            )}
          ...
          >
            ...
            <div
              style={{
                ...
                border: "1.5px solid rgba(92,61,46,0.06)",
              }}
            >
              <div
                style={{
            ...
            <div
              style={{
                flex: 1,
                background: "linear-gradient(135deg, var(--bg-admin), #2d1a0a)",
                padding: "24px",
                borderRadius: "var(--radius-lg)",
            ...
            >
              ...
              <select
                style={{
                  ...
                  border: "1.5px solid rgba(201,149,106,0.25)",
                ...
                <button
                  ...
                  style={{
                    background:
                      "linear-gradient(135deg, var(--coffee-latte), var(--coffee-caramel))",
                    color: "var(--coffee-espresso)",
                    boxShadow: "0 4px 16px rgba(201,149,106,0.35)",
                  }}
                >
                  ✅ Kirim Pesanan
                </button>
        ...
        {activeTab === "reservations" && (
          <div className="inventory-section">
            ...
            >
              ...
              <button
                className="btn-ready"
                onClick={() => setShowManualResv(true)}
              >
                + Tambah Manual
              </button>
            </div>
            <div className="order-grid">
              {reservations
                ...
                .map((resv) => (
                  ...
                  >
                    <div className="order-card-header">
                      <h3>{resv.customer_name}</h3>
                      <span
                        className={`status-badge badge-${resv.status === "pending_wa" ? "warning" : "success"}`}
                      >
                    ...
                    <div className="order-card-body">
                      <div style={{ marginBottom: "12px" }}>
                        {resv.reservation_type === "event" ? (
                          ...
                          >
                            🎟️ EVENT ·{" "}
                            {resv.event_choice?.startsWith("paket")
                              ? "PAKET"
                              : resv.event_choice?.startsWith("pilih")
                                ? "PILIHAN MENU"
                                : "BELUM PILIH"}
                          </span>
                        ) : (
                          <span
                            style={{
                              background:
                                "linear-gradient(135deg, var(--accent-info), #1560a0)",
                              color: "white",
                              padding: "4px 12px",
                              borderRadius: "999px",
                              fontSize: "0.75rem",
                      ...
                      <p>Kontak: {resv.customer_phone}</p>
                      <p>
                        Jumlah: <strong>{resv.pax_count} Orang</strong>
                      </p>
                      <p>
                        Plot:{" "}
                        <strong>
                          {resv.table_id ? `Meja ${resv.table_id}` : "Belum"}
                        </strong>
                      </p>
                      <h4 className="order-total">
                        {new Date(resv.reservation_date).toLocaleString(
                          "id-ID",
                        )}
                      </h4>
                    </div>
                      ...
                      {resv.status === "confirmed" &&
                        (resv.reservation_type === "event" ? (
                          <button
                            ...
                            onClick={() => handleOpenPrintModal(resv)}
                          >
                            🖨️ Event Selesai &amp; Cetak
                          </button>
                        ) : (
                          <div
                            style={{
                          ...
                          >
                            <button
                              className="btn-process"
                              onClick={() => completeMejaBiasa(resv.id)}
                            >
                              🚶‍♂️ Tamu Datang &amp; Selesai
                            </button>
                            <p
                              style={{
                                margin: 0,
                            ...
                            >
                              Tagihan makan diproses via
                              <br />
                              <strong>Live Orders</strong> atau{" "}
          ...
          >
            ...
            <div
              style={{
                ...
                justifyContent: "space-between",
                alignItems: "center",
                boxShadow: "0 20px 60px rgba(15,52,96,0.3)",
              }}
            >
              <div>
                <p
              ...
              <>
                ...
                >
                  ...
                  >
                    ...
                    >
                      ✓ Sudah termasuk semua pembayaran
                    </p>
                  </div>
                  {/* Card Transaksi */}
                  <div
                  ...
                  >
                    ...
                    >
                      Rp{" "}
                      {reportData.total_discount_given.toLocaleString("id-ID")}
                    </h2>
                    {reportData.total_revenue > 0 && (
                      ...
                      >
                        {(
                          (reportData.total_discount_given /
                ...
                >
                  ...
                  >
                    ...
                    >
                      Berdasarkan jumlah porsi terjual hari ini
                    </p>
                    {reportData.top_items.length === 0 ? (
                      ...
                      (() => {
                        const maxQty = Math.max(
                          ...reportData.top_items.map((i) => i.qty),
                        );
                        const colors = [
                          "#6f4e37",
                          "#a0522d",
                            ...
                            >
                              <div
                                style={{
                                  ...
                                  background: `linear-gradient(90deg, ${colors[idx] || "#d4a96a"}, ${colors[idx] || "#d4a96a"}cc)`,
                                  borderRadius: "20px",
                    ...
                    >
                      <div>
                        ...
                        <p style={{ color: "#aaa", fontSize: "0.8rem" }}>
                          {reportData.orders_detail?.length || 0} transaksi
                          tercatat
                        </p>
                      </div>
                    </div>
                    <div style={{ overflowY: "auto", maxHeight: "320px" }}>
                      {!reportData.orders_detail ||
                      reportData.orders_detail.length === 0 ? (
                        <div
                          style={{
                            textAlign: "center",
                            padding: "40px 0",
                          ...
                          <tbody>
                            {reportData.orders_detail.map((ord, idx) => (
                              ...
                              >
                                ...
                                >
                                  Meja{" "}
                                  {ord.tables?.table_number || ord.table_id}
                                </td>
                                <td
                                  style={{
                                    padding: "12px 14px",
                                ...
                                >
                                  Rp{" "}
                                  {Number(ord.final_total).toLocaleString(
                                    "id-ID",
                                  )}
                                </td>
                              </tr>
        ...
        {activeTab === "inventory" && (userRole === "admin" || userRole === "owner") && (
          <div className="inventory-section">
            ...
            >
              ...
              <button
                className={showInvHistory ? "btn-secondary" : "btn-process"}
                  ...
                  {inventory.map((item) => (
                    ...
                    >
                      <td>
                        {item.name}{" "}
                        {item.current_stock <= item.min_stock_level && (
                          <span className="alert-badge">⚠️ Stok Menipis</span>
                        )}
                      </td>
                      <td>
                ...
                <tbody>
                  ...
                  {invHistory.map((hist) => (
                    <tr key={hist.id}>
                      ...
                      <td>
                        <strong>{hist.raw_materials?.name}</strong>
                      </td>
                      <td>
                        <span
                          style={{
                            color:
                              hist.new_stock > hist.old_stock ? "green" : "red",
                            fontWeight: "bold",
                          }}
                        >
            ...
            <table className="inventory-table">
              ...
              <tbody>
                {promotions.map((promo) => (
                  <tr key={promo.id}>
                    <td>
                      <strong>{promo.menu_items?.name}</strong>
                    </td>
                    <td>{promo.name}</td>
                    <td>
                      {promo.type === "percent"
              ...
              <tbody>
                {usersList.map((u) => (
                  <tr key={u.id}>
                    ...
                    <td>
                      <span
                        className={`status-badge ${u.role === "admin" ? "badge-warning" : "badge-preparing"}`}
                      >
                        {u.role.toUpperCase()}
                      </span>
                    </td>
